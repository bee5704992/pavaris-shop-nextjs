...
const Cart = () => {
  ...
  useEffect(() => {
    const cartLocal = JSON.parse(localStorage.getItem('cart_pavaris_shop'))
    if (cartLocal.length && cartLocal.length > 0) {
      let newArr = []
  ...
  if (cart.length === 0) return <img className="img-reponsive w-100" src="/empty_cart.jpg" alt="empty cart!" />
  return (
