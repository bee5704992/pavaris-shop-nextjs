                    ...
                    <div key={category._id} className="card my-2 text-capitalize">
                        <div className="card-body d-flex justify-content-between">
                            ...
                            <div style={{cursor: 'pointer'}}>
                                <i className="fas fa-edit mr-2 text-info" aria-hidden
                                onClick={() => handleEdit(category)}></i>

                                <i className="fas fa-trash-alt text-danger" aria-hidden
                                 data-toggle="modal" data-target="#exampleModal"
                                 onClick={() => dispatch({
                                     type: 'ADD_MODAL',
                                     payload: [{
                                         type: 'ADD_CATEGORIES', data: categories, id: category._id, 
                                         title: category.name, bodytext: 'Do you want to delete this category?'
                                        }]
                                 })}></i>
                            </div>
                        </div>
