...
const ProductManager = () => {
    ...
    const [resProduct, setResProduct] = useState(initialState)
    const {       
        title,
        price,
        inStock,
    ...
    useEffect(()=>{
        if(id){
            ...
            getData(`product/${id}`).then(res => {
                if(res.err){
                    if(res.err_name === 'CastError'){
                        router.push('/')
                        return dispatch({type: 'NOTIFY', payload: {error: 'Invalid product ID.'}})
                    }
                    router.push('/')
                ...
                setResProduct(res.product)
                setImages(res.product.images)
                setResImages(res.product.images)
            })
        }else{
            setOnEdit(false)
    ...
    return(
        <div className="product_manager">
            ...
            <form className="row" onSubmit={handleSubmit}>
                <div className="col-md-6">
                    ...
                    <div className="row">
                        ...
                        <div className="col-sm-6">
                            ...
                            placeholder="Count in stock" className="d-block w-100 p-2" 
                            onChange={handleChange} />
                        </div>
                    </div>
                    ...
                    <div className="input-group-prepend px-0 my-2">
                        ...
                        onChange={handleChange} >
                            <option value="all">Please choose 1 category.</option>
                            {
                                categories.map(item => (
                ...
                <button type="submit" className="btn btn-info my-2 px-4 mx-3" disabled={handleDisableSubmit()}>{id ? 'Update' : 'Create'}</button>
            </form>
